				...
				<div style="text-align: center;" class="fresh-table full-color-orange">
					<table style="zoom: 1.5;" id="fresh-table" class="table">
						<thead>
							...
							<th data-field="subject">게시글</th>
							<th data-field="email" data-sortable="true">작성자</th>
						</thead>
						<tbody>
							<%

								ArrayList<Counsel> counselArr = counselDAO.selectCounselAll();
							%>
							<c:forEach var="counsel" items="<%=counselArr%>" varStatus="status">
								<tr>
									<th scope="row" class="scope">${status.count}</th>
									<td><a href="./counselDetail.jsp?num=${counsel.getsNum()}">${counsel.getsSubject()}</a></td>
									<td>${counsel.getsUseremail()}</td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
